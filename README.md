...
回帰結果の表を渡した場合、列名の `（1）\n被説明変数` から**被説明変数も取り出して先頭に並べ**、そのあとにインデックスの説明変数が続きます。モデルが複数あって被説明変数が同じでも、重複は除かれます。`定数項` 以降の統計量（`決定係数`・`自由度修正済み決定係数`・`観測数` など）と、標準誤差の行（インデックスが `NaN` や空文字）は自動的に除外されます。

戻り値は **2 つ**で、データソース表と、渡した表に注釈行を足したものです。

```python
from kzemi_tools import generate_data_source

data_source, df_result_modified = generate_data_source(df_result, units_df)

data_source
#              変数    単位                                              出典
# 0  財政力指数（都道府県財政）   ‐  eStat「都道府県・市区町村のすがた(社会・人口統計体系)」データベース
# 1            総人口    人  eStat「都道府県・市区町村のすがた(社会・人口統計体系)」データベース
# 2      最低賃金（時間額）    円                       厚生労働省「地域別最低賃金の全国一覧」
```

`df_result_modified` は `df_result` の最終行（`観測数` など）の下に、**第 1 列だけに注釈文**を入れた行を足した表です。1）は被説明変数と単位から自動生成され（単位がない変数には単位を書きません）、2）3）は固定の文言です。

```python
print(df_result_modified.iloc[-1, 0])
# （注）1）被説明変数は「空き家数」（単位：戸）、「財政力指数」、「年平均気温」（単位：ﾟC）。
# 　　　2）カッコ内は不均一分散頑健な標準誤差。
# 　　　3）*** は1%,** は5%,* は10% の有意水準で棄却されることを表す。
```

> 記述統計量の表（`df.describe().T` など）を渡した場合、列名が `（1）\n…` の形式ではないので被説明変数は追加されず、インデックスの変数だけが対象になります。このときは被説明変数が取れないため、2 つめの戻り値は注釈行を足さずにそのまま返します。

`compact=True` にすると、出典を出現順に `a`, `b`, `c`… の記号で表示し、対応を示す注釈文が表の最終行（`変数` 列）に入ります。出典が長いときに表を読みやすくするためのオプションで、そのままコピーして論文の表に貼り付けられます。

```python
data_source, df_result_modified = generate_data_source(df_result, units_df, compact=True)
#                                                  変数    単位  出典
# 0                                               総人口   万人   a
# 1                                             年平均気温  ﾟC   b
# 2  （注）1）データの出典について、aはeStat「都道府県・市区町村のすがた(社会・人口統計体系)...
```
...
| 引数         | 型          | 説明                                                                         |
| ------------ | ----------- | ---------------------------------------------------------------------------- |
| `df_summary` | `DataFrame` | 記述統計量または回帰結果の DataFrame（インデックスが変数名、回帰結果なら列名が `（1）\n被説明変数`） |
| `units`      | `DataFrame` | `clean_estat_csv` で取得した `変数名`・`単位`・`出典` の 3 列を持つ DataFrame |
| `compact`    | `bool`      | `True` で出典を `a`, `b`, `c`… の記号にし、注釈文を最終行に追加（デフォルト: `False`） |

**戻り値** — `(data_source, df_summary_modified)` のタプル

| 戻り値                | 型          | 説明                                                                                         |
| --------------------- | ----------- | -------------------------------------------------------------------------------------------- |
| `data_source`         | `DataFrame` | `変数`, `単位`, `出典` の 3 列。`compact=True` のときは最終行が注釈文                          |
| `df_summary_modified` | `DataFrame` | 渡した表の最終行の下に注釈行を足したもの（被説明変数が取れない表ならそのまま）                 |
